...
tablesvm
class_accsvm1 <- tablenn[1,1]/(tablenn[1,1] + tablenn[1,2])
class_accsvm2 <- tablenn[2,1]/(tablenn[2,1] + tablenn[2,2])
(class_accsvm1 + class_accsvm2)/2 #average class accuracy
1/((1/class_accsvm1)+(1/class_accsvm2)) #harmonic mean
mean(result_test_feature_selection == test.data$V25) #accuracy
auc(test.data$V25, predtest) #area under roc curve
tablesvm[2,2]/(tablesvm[2,1] + tablesvm[2,2]) #specificity
...
#BEFORE BALANCING
#TRAIN
nn <- nnet(train.data$V25 ~ V7 + V13 + V8, train.data[,1:24],maxit=1000,size=8,entropy=TRUE, decay = 0.01)
...
mean(test.binpred == test.data$V25) #accuracy
auc(test.data$V25, predtest) #area under roc curve
tablenn[2,2]/(tablenn[2,1] + tablenn[2,2]) #specificity
...
mean(tree.predict_test == test.data$V25) #accuracy
auc(test.data$V25, as.numeric(tree.predict_test)) #area under roc curve
tabletree[2,2]/(tabletree[2,1] + tabletree[2,2]) #specificity
