...
#TRAIN
tree.model_undersample <- rpart(V25 ~ ., data = undersampled_train_data, method = "class", minbucket = 20)
tree.predict_undersampletrain <- predict(tree.model_undersample, undersampled_train_data, type = "class")
tree.binpredict_undersampletrain <- predict(tree.model_undersample, undersampled_train_data)
tree.ROCR_undersampletrain <- prediction(tree.binpredict_undersampletrain[,2], undersampled_train_data$V25)
tree.auc_undersampletrain <- as.numeric(performance(tree.ROCR_undersampletrain,"auc")@y.values)
tree.auc_undersampletrain

#TEST
tree.predict_undersample <- predict(tree.model_undersample, test.data, type = "class")
tree.binpredict_undersample <- predict(tree.model_undersample, test.data)
tree.ROCR_undersample <- prediction(tree.binpredict_undersample[,2], test.data$V25)
tree.auc_undersample <- as.numeric(performance(tree.ROCR_undersample,"auc")@y.values)
tree.auc_undersample
...
#UNDERSAMPLE
#TRAIN
nn_undersample <- nnet(V25~., undersampled_train_data,maxit=2000,size=30,entropy=TRUE, decay = 0.005)
...
#TEST
predundertest <- predict(nn_undersample, newdata = test.data)
testunder.binpred <- predict(nn_undersample, newdata = test.data, type = c("class"))
mean(test.data$V25 == testunder.binpred)
auc(test.data$V25, predundertest)
```
...
#logistic model
log_model <- glm(V25 ~ V3+V4+V5+V7+V8+V9+V10+V11+V12, data = train.data, family = "binomial")
summary(log_model)

predict <- predict(log_model, test.data, type = "response")
predict <- ifelse(predict >0.25, 1, 0)
#confusion matrix
